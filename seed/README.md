...
```powershell
Get-Content "$HOME\Projects\weather-rp2040-lab-materials\seed\power_realtime_seed.sql" -Raw | docker exec -i weather-mysql mysql -u weather -pweatherpass weather
```

PowerShell에서는 `< 파일` 리다이렉션이 되지 않는다. 경로는 `power_realtime_seed.sql` 위치에 맞게 바꾼다.
